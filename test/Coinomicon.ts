...
describe("Coinomicon", function () {
    ...
    describe("Deployment", function () {
        it("Deploy and create new exchange", async function () {
            const { coinomicon, owner, account1, account2 } = await loadFixture(
                deployFixture
            );
        ...
        it("Bad tests", async function () {
            ...
            await expect(
                coinomicon.createExchange(token.address)
            ).to.be.revertedWith("Exchange already exists");
            await expect(
                coinomicon.createExchange(
    ...
    describe("Orders", function () {
        ...
        it("Create sell orders", async function () {
            ...
            expect(
                (await exchange.marketSellPrice(100))._available
        ...
        it("Buy market price", async function () {
            ...
            for (let i = 0; i < 100; i++) {
                ...
                await token
                    .connect(account2)
                    .approve(exchange.address, 150 + i);
                if (i % 2 == 0)
                    ...
                    await expect(
                        exchange.connect(account2).sellLimit(150 + i, i * i)
                    )
                        .to.emit(exchange, "SellOrderCreated")
                        .withArgs(i, 150 + i, i * i);
            }
            ...
            const initialOwnerTokenBalance = await token.balanceOf(
                owner.address
            );
            const [available, paymentAmount] = await exchange.marketSellPrice(
                10000
            );
            expect(available).to.be.greaterThanOrEqual(10000);

            for (let i = 0; i < 10; i++) {
                const [available, _paymentAmount] =
                    await exchange.marketSellPrice(1000);
                await exchange
                    .connect(account1)
                    .buyMarket(1000, { value: _paymentAmount });
            }
