...
describe("Coinomicon", function () {
	...
	describe("Deployment", function () {
		it("Deploy and create new exchange", async function () {
			...
			await expect(coinomicon.createExchange(token.address)).to.emit(coinomicon, "ExchangeCreated");
		});
		...
		it('Bad tests', async function () {
			...
			await expect(coinomicon.createExchange(token.address)).to.be.revertedWith("Exchange already exists");
			await expect(coinomicon.createExchange("0x0000000000000000000000000000000000000000")).to.be.revertedWith("Invalid token address");
		});
	});

	describe("Orders", function () {
		it("Create buy orders", async function () {
			const { coinomicon, exchange, token, owner, account1, account2 } = await loadFixture(createExchangeFixture);

			for (let i = 0; i < 100; i++) {
				await expect(exchange.buyLimit(i*100, i*i, { value: i*100*i*i })).to.emit(exchange, "BuyOrderCreated").withArgs(i, i*100, i*i);
			}
		});
		...
		it("Create sell orders", async function () {
			...
			for (let i = 0; i < 100; i++) {
				...
				await expect(exchange.sellLimit(15 + i, i)).to.emit(exchange, "SellOrderCreated").withArgs(i, 15 + i, i);
			}
		...
		it("Buy market price", async function () {
			const { coinomicon, exchange, token, owner, account1, account2 } = await loadFixture(equalTokenBalanceFixture);

			for (let i = 0; i < 100; i++) {
				await token.approve(exchange.address, 150 + i);
				await token.connect(account2).approve(exchange.address, 150 + i);
				if (i % 2 == 0) await expect(exchange.sellLimit(150 + i, i*i)).to.emit(exchange, "SellOrderCreated").withArgs(i, 150 + i, i*i);
				else await expect(exchange.connect(account2).sellLimit(150 + i, i*i)).to.emit(exchange, "SellOrderCreated").withArgs(i, 150 + i, i*i);
			}
